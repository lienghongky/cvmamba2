...
from modelkit.utils.registry import ARCH_REGISTRY
...
##########################################################################
## Overlapped image patch embedding with 3x3 Conv
class OverlapWordPatchEmbed(nn.Module):
    def __init__(self, in_c=3, embed_dim=48, bias=False):
        super(OverlapWordPatchEmbed, self).__init__()

        # Initial convolutional layer
        self.proj1 = nn.Conv2d(in_c, embed_dim, kernel_size=2, stride=1, padding=1, bias=bias)

        # Additional convolutional layers for capturing more local information
        self.proj2 = nn.Conv2d(embed_dim, embed_dim, kernel_size=2, stride=1, padding=1, bias=bias)
        self.proj3 = nn.Conv2d(embed_dim, embed_dim, kernel_size=2, stride=1, padding=1, bias=bias)
        self.proj4 = nn.Conv2d(embed_dim, embed_dim, kernel_size=2, stride=1, padding=1, bias=bias)
        self.linear = nn.Linear(embed_dim,48,bias=bias)
    def forward(self, x):
        x = self.proj1(x)
        x = F.relu(x)
        x = self.proj2(x)
        ...
        # Rearrange to maintain the original shape format
        x = rearrange(x, "b c h w -> b (h w) c").contiguous()
        x = self.linear(x)
        # Crop or pad the output to the desired size
        if x.shape[1] > 65536:
            x = x[:, :65536, :]
        elif x.shape[1] < 65536:
            padding = 65536 - x.shape[1]
...
class CVMambaIR(nn.Module):
    def __init__(self,
        ...
        super(CVMambaIR, self).__init__()
        self.mlp_ratio = mlp_ratio
        self.patch_embed = OverlapPatchEmbed(inp_channels, dim)
        self.word_patch_embed = OverlapWordPatchEmbed(inp_channels, dim )
        base_d_state = 4
        self.word_encoder_level1 = nn.ModuleList([
            VSSBlock(
                hidden_dim=dim,
        ...
        #self.down_word_1_2 = Downsample(dim)  ## From Level 1 to Level 2
        self.word_encoder_level2 = nn.ModuleList([
            VSSBlock(
        ...
        self.word_encoder_level3 = nn.ModuleList([
            VSSBlock(
        ...
        # self.word_latent = nn.ModuleList([
        #     VSSBlock(
        ...
        # self.word_decoder_level3 = nn.ModuleList([
        #     VSSBlock(
        #         hidden_dim=int(dim * 2 ** 2),
        #         drop_path=drop_path_rate,
        #         norm_layer=nn.LayerNorm,
        ...
        # self.word_decoder_level2 = nn.ModuleList([
        #     VSSBlock(
        #         hidden_dim=int(dim * 2 ** 1),
        #         drop_path=drop_path_rate,
        #         norm_layer=nn.LayerNorm,
        ...
        # self.word_decoder_level1 = nn.ModuleList([
        #     VSSBlock(
        #         hidden_dim=int(dim * 2 ** 1),
        #         drop_path=drop_path_rate,
        #         norm_layer=nn.LayerNorm,
    ...
    def forward(self, inp_img):
        ...
        #forward level 1
        inp_word_enc_level1 = self.word_patch_embed(inp_img)
        out_word_enc_level1 = inp_word_enc_level1
        for layer in self.word_encoder_level1:
            out_word_enc_level1 = layer(out_word_enc_level1, [H, W ])

        for layer in self.encoder_level1:
            out_enc_level1 = layer(out_enc_level1, [H, W])

        #forward level 2
        inp_word_enc_level2 = self.down1_2(out_word_enc_level1, H , W )  # b, hw//4, 2c
        out_word_enc_level2 = inp_word_enc_level2
        for layer in self.word_encoder_level2:
            out_word_enc_level2 = layer(out_word_enc_level2, [H // 2, W // 2])
        ...
        #forward level 3
        inp_word_enc_level3 = self.down2_3(out_word_enc_level2, H // 2, W // 2)  # b, hw//16, 4c
        out_word_enc_level3 = inp_word_enc_level3
        for layer in self.word_encoder_level3:
            out_word_enc_level3 = layer(out_word_enc_level3, [H // 4 , W // 4])
        ...
        out_enc_level3 = out_enc_level3 + out_word_enc_level3
        #forward level 4
        # inp_word_enc_level4 = self.down3_4(out_word_enc_level3, H // 4, W // 4)  # b, hw//64, 8c
        # word_latent = inp_word_enc_level4
        # for layer in self.latent:
        #     latent = layer(word_latent, [H // 8, W // 8])
